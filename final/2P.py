...
# scene_info["platform_2P"][-2] platform_center_x
# scene_info["platform_2P"][-1] platform_center_y
        ...
        if not self.ball_served:    #如果未發球
            ...
            if(len(ball_position_history)>1):
                ...
                if(ball_down != True and m>0):
                    if(ball_position_history[-1][-1]>275): #如果球在Y=0~200這裡反彈，下一次反彈會超過板子，不用再計算球的落點位置，只計算Y=200~500區間的彈跳
                        temp_y=ball_position_history[-1][-1]-abs((ball_position_history[-1][-2]/scene_info["ball_speed"][-2])*scene_info["ball_speed"][-1]) #第一次反彈的Y座標
                        temp_x=0    #第一次反彈的X座標
                        temp_y2=temp_y-abs((195/scene_info["ball_speed"][-2])*scene_info["ball_speed"][-1]) #第二次反彈的Y座標
                        temp_x2=195 #第二次反彈的X座標
                        temp_x3=abs(temp_x2-abs(((temp_y2-80)/scene_info["ball_speed"][-1])*scene_info["ball_speed"][-2]))    #球最後的落點X座標
                        temp_y3=80  #球最後的落點Y座標,因為P2板子Y座標在50，板子高度30
                        
                if(ball_down != True and m<0):    #同上,m=-1的情況
                    if(ball_position_history[-1][-1]>275):
                        temp_y=ball_position_history[-1][-1]-abs((195-ball_position_history[-1][-2])/scene_info["ball_speed"][-2]*scene_info["ball_speed"][-1])
                        temp_x=195
                        temp_y2=temp_y-abs((195/scene_info["ball_speed"][-2])*scene_info["ball_speed"][-1])
                        temp_x2=0   
                        temp_x3=abs(temp_x2-abs(((temp_y2-80)/scene_info["ball_speed"][-1])*scene_info["ball_speed"][-2]))
                        temp_y3=80  
                ...
                if(ball_down ==True):     #擊球後，板子回到中心             
                    if(scene_info["platform_2P"][-2]+20<100):
                    ...
                    if(scene_info["platform_2P"][-2]+20<temp_x3):   #板子中心<球最後落點
                        return "MOVE_RIGHT" #板子右移
                    if(scene_info["platform_2P"][-2]+20>temp_x3):#板子中心>球最後落點
                        return "MOVE_LEFT" #板子左移
                    else:
